 ...
 let AudioPlayer = (function() {
		  ...
		  function beforeUnload(evt) {
			if(!audio.paused) {
			  let message = 'Music still playing';
		 ...
		 */
			...
			function listHandler(evt) {
			  ...
			  if(evt.target.matches('.pl-list__title')) {
				let current = parseInt(evt.target.closest('.pl-list').getAttribute('data-track'), 10);
				if(index !== current) {
				  index = current;
				  play(current);
				}
				  ...
				  if(!!evt.target.closest('.pl-list__remove')) {
					let parentEl = evt.target.closest('.pl-list');
					let isDel = parseInt(parentEl.getAttribute('data-track'), 10);
			...
			function plActive() {
			  ...
			  let current = index;
			  for(let i = 0, len = plLi.length; len > i; i++) {
		 ...
		 */
		  ...
		  function timeUpdate() {
			...
			if(settings.buffered) {
			  ...
			  if(buffered.length) {
				let loaded = Math.round(100 * buffered.end(0) / audio.duration);
		  ...
		  function notify(title, attr) {
			...
			window.Notification.requestPermission(function(access) {
			  if(access === 'granted') {
				let notice = new Notification(title.substr(0, 110), attr);
       ...
       module.exports={AudioPlayer}
		...
		// let iconImage = 'http://funkyimg.com/i/21pX5.png';
